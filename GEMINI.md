...
8. [x] **Módulo de Análise de Crédito Comercial & Motor de Risco (`analise_credito_engine.js`, `postgres_db.js`, `server.js`, `public/app.js`):**
   ...
   - **Auditoria e Identidade do Analista Responsável:** Persistência automática do usuário logado (via token JWT ou payload) na tabela `analise_credito_history` (`usuario VARCHAR(100)`), dados_completos e JSON de fallback. Exibição da identificação do analista no cabeçalho da Ficha do Pedido (`Empresa: XX | Cliente: ... | Data: ... | Usuário: <nome/login>`) e suporte a busca por operador no filtro do histórico.
...
4. **Health Check e Degradao Graciosa:** Criar endpoints de monitoramento de status para checar conectividade com PostgreSQL, APIs de terceiros e disponibilidade de pastas de integracao.
