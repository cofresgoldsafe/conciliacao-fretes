...
10. [x] **Sub-aba Pedidos Compras no Módulo Vendedores (`protheus_db.js`, `server.js`, `public/index.html`, `public/app.js`, `test_pedidos_compras.js`):**
   ...
   - **Filtro Estrito de Produtos PA & Faixa de Códigos:** Integração com a tabela mestra de produtos `SB1010`, filtrando exclusivamente produtos acabados (`B1_TIPO = 'PA'`) e com códigos entre `001000000000000` e `019999999999999` tanto na query T-SQL do backend quanto na camada reativa do frontend.
   ...
   - **Busca Instantânea & Métricas:** Filtro instantâneo conforme digitação por produto, código, pedido ou fornecedor, filtro por empresa, cards de métricas (Itens em aberto, saldo total e previsão mais próxima), ordenação de 4 colunas e 10 testes automatizados.
